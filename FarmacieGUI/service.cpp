...
void Service::add_medicament(const string& den, const string& prod, const string& subst, float pret )
{
	Medicament med = Medicament(den, prod, subst, pret);
	valid.validare_med(med);
	all.add_medicament(med);
	undoActions.push_back(std::make_unique<UndoAdauga>(all,med));
}

void Service::delete_medicament(const string& denumire) {
	valid.validare_den(denumire);

	Medicament m = find_medicament(denumire);
	undoActions.push_back(std::make_unique<UndoSterge>(all, m));
...
vector<Medicament>& Service::get_all() const  {
	return all.get_all();
}
...
void Service::update_medicament(const string& denumire, const Medicament& med) {
	...
	undoActions.push_back(std::make_unique<UndoModifica>(all, m, med));
...
vector<Medicament> Service::filter_subst(const string& substanta){
	vector<Medicament> rez;
	std::copy_if(all.get_all().begin(), all.get_all().end(), std::back_inserter(rez), [substanta](const Medicament& med) {
		return med.get_substanta() == substanta;
		});
...
vector<Medicament>& Service::all_meds()
{
	return reteta.all_meds();
}
...
void Service::exporteaza(const string& fisier)
{
	reteta.export_reteta(fisier);
}